...
export default class SPlaceService extends AuthService {
    constructor() {
        super();
    }
    ...
    addSafePlace(drawingVo) { 
        return axios.post(url.ADD_SAFE_PLACE, drawingVo, super.setTokenToRequest())
            .then((response) => {
    ...
    getDrawing(eventId) {
        return axios.get(url.GET_SAFE_PLACE_DRAWING + eventId, super.setTokenToRequest())
            .then((response) => {
                console.log(response);
                return response;
            }).catch((error) => {
